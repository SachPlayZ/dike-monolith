...
- [x] Add fee sponsorship only in `dike-services` and `dike-web`; no Soroban contract or deployment changes.
- [x] Keep the connected G-account as the inner transaction source. The wallet signs the assembled Soroban transaction; the service pays only the fee through a fee-bump envelope.
- [x] Sponsor one `invokeHostFunction` operation targeting an allowlisted Dike contract/method from the active deployment manifest.
- [x] Use a server-only sponsor seed behind a signer interface. Fail startup when sponsorship is enabled with invalid/missing key material. Never send or log the seed.
- [x] Use an explicit rollout flag. When sponsorship is enabled, failures are shown to the user; never silently fall back to charging the wallet. When disabled, retain the current direct-submit path.
- [x] Protect funds with XDR/body limits, source-signature and time-bound checks, contract/method allowlists, fee/resource caps, per-IP/per-source quotas, a conservative daily declared-fee budget, and replay protection in Redis.
- [x] Return the outer fee-bump hash and base64 Soroban return-value XDR so existing UI behavior, including council reward decoding, remains intact.
- [x] V1 limitation: standard G-account source signatures only. Multisig/custom signer policy support is a follow-up.
...
- [x] **01 — `document fee sponsorship architecture`**
...
- [x] **02 — `configure fee sponsorship`**
...
- [x] **04 — `define sponsorship errors`**
...
- [x] **05 — `validate sponsorship payloads`**
...
- [x] **06 — `parse sponsored inner transactions`**
...
- [x] **07 — `verify inner source signatures`**
...
- [x] **08 — `allowlist sponsored contract calls`**
...
- [x] **09 — `enforce sponsorship fee limits`**
...
- [x] **11 — `prevent sponsored transaction replay`**
...
- [x] **13 — `submit sponsored transactions`**
...
- [x] **14 — `orchestrate fee sponsorship`**
...
- [x] **15 — `expose sponsored transaction api`**
...
- [x] **16 — `instrument fee sponsorship`**
...
- [x] **17 — `report sponsorship availability`**
...
- [x] **18 — `proxy sponsorship requests`**
...
- [x] **19 — `add sponsorship web client`**
...
- [x] **20 — `prepare inner transactions for sponsorship`**
...
- [x] **21 — `show gasless signing state`**
...
- [x] **22 — `centralize transaction execution`**
...
- [x] **23 — `sponsor trading transactions`**
...
- [x] **24 — `sponsor remaining app transactions`**
...
- [x] **25 — `verify fee sponsorship end to end`**
...
- [x] Confirm exactly 25 focused commits; every commit builds and includes its relevant tests.
- [ ] Services: `npm test`, `npm run lint`, `npm run build`, `npm run audit` (audit still reports existing `fast-uri`/`find-my-way` high advisories).
- [ ] Web: `pnpm test`, `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm build`, `pnpm audit --prod --audit-level high` (audit still reports existing transitive advisories).
- [x] Security: tampered/wrong-network/expired/disallowed/oversized/high-fee/replayed requests never reach sponsor signing.
- [x] Funds: quota and daily budget remain atomic under concurrency; sponsor exhaustion degrades only sponsorship.
...
- [x] UX: all write paths show sponsoring/pending/success/failure accurately and never silently charge the wallet after sponsor failure.
- [x] Review final diff for secret exposure, full-XDR logging, unrelated refactors, contract changes, and deployment-manifest changes.
...
- [x] Production signer backend: use runtime-injected seed behind the replaceable signer interface for v1; KMS/HSM remains a follow-up.
- [x] Final per-source/per-IP quotas, daily XLM budget, max resource fee, and outer inclusion bid: use the checked-in conservative testnet defaults and tune before mainnet.
- [x] Sponsor all allowlisted admin/timelock writes; their existing wallet/contract authorization remains required.

## Review

### Changed

- Implemented the 25-commit fee-bump sponsorship plan across `dike-services` and `dike-web`.

### Verified

- Services: 26 test files / 83 tests pass; lint and production build pass.
- Web: 6 test files / 21 tests pass; lint, typecheck, and production build pass.
- Confirmed no contract or deployment-manifest change; exactly 25 feature commits are present.

### Risks

- Sponsor-key custody is runtime seed based in v1; mainnet use still requires operational secret management.
- V1 deliberately excludes multisig/custom source-signature policies.
- Dependency audits still report high-severity advisories outside this feature's source changes.

### Follow-ups

- Run the real local/testnet smoke with a funded sponsor and wallet-signed assembled inner XDR before enabling production sponsorship.
